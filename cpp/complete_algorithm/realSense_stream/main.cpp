	...
	while (psm->AcquireFrame(true) >= PXC_STATUS_NO_ERROR) {
		...
		for (pxcU16 i = 0; i < numOfFaces; i++)
		{
			...
			if (trackedFace != NULL)
			{
				...
				/********************************************************************************************************/
				/*
				PXCFaceData::PulseData* pulseData = trackedFace->QueryPulse();
				pxcBool pulseExist;
				pxcF32 heartRate;
				//PXCFaceData::PoseData::QueryHeartRate
				if (pulseData == NULL)
					pulseExist = 0;
				else
					pulseExist = pulseData->QueryHeartRate();

				//wprintf_s(L"heartRate data--- %d \n", (int)heartRate);
				wprintf_s(L"heartRate data--- %d \n", (int)pulseExist);
				*/
				/********************************************************************************************************/
				...
				if (poseAnglesExist) {
					if (poseData->QueryConfidence() > 0){
						//wprintf_s(L"pitch data--- %d \n", (int)angles.pitch); //lookup or down // 15 - -5
						//wprintf_s(L"roll data--- %d \n", (int)angles.roll); //lean to the side // 6 - -6
						//wprintf_s(L"yaw data--- %d \n\n", (int)angles.yaw);//10 - -10 //look to the side
				...
				if (landmarkData != NULL)
				{
					...
					if (landmarkData->QueryPoints(landmarkPoints)) {
						...
						//if (true) {
							...
							if (controller->confidenceFlag) {
								//wprintf_s(L"111111111111111111111111111111111111");
								//call farther testing????
								//call calculate function
								confidenceCounter++;
								if (confidenceCounter == 10) {
									//controller->calculatePointDistances(landmarkPoints,distances[0],landmarkPoints[0].world.x, landmarkPoints[0].world.y, landmarkPoints[0].world.z);
									controller->calculateAllDistances(landmarkPoints);

								}
							}
							else {
								//wprintf_s(L"0000000000000000000000000000000000000");
								confidenceCounter = 0;
							}
						//}
					}

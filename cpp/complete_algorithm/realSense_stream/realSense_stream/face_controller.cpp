...
void FaceController::calculateAllDistances(PXCFaceData::LandmarkPoint *_points) {
	...
	*/
	ofstream outfile;
	outfile.open("D:/degree/final project/documents/result5.csv");

	std::string finalbinary = "";
	for (int i = 0; i<78; i++)
	{
		outfile << distances[i][0];
		for (int j = 0; j < 78; j++) {
			int temp = (int)(distances[i][j] * 1000000);
			//wprintf_s(L"found landmark 0 - %d\n", temp);
			std::string tempbinary = std::bitset<32>(temp).to_string();
			//cout << tempbinary << '\n';
	...
	//wprintf_s(L"found landmark 0 - %s\n", finalbinary.c_str());
	cout << finalbinary << '\n';

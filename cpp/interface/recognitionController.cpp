...
/*void RecognitionController::SetMaxFaces(int _maxFaces)
{
	maxFaces = _maxFaces;
...
void RecognitionController::SetLandmark(pxcI32 _faceId, PXCFaceData::LandmarkPoint *_points, pxcI32 _numofPoints)
{
	int sum = 0;
	confidenceFlag = true;
...
boolean RecognitionController::calculateDistances() {
	return true;
}

boolean RecognitionController::TestLandmarkDepth(pxcI32 _faceId, PXCFaceData::LandmarkPoint *_points, pxcI32 _numofPoints)
{
	if (!faces[_faceId].landmarksEnabled)
	{
		//faces[_faceId].landmarksEnabled = true;
...
void RecognitionController::calculateAllDistances(PXCFaceData::LandmarkPoint *_points) {
...
void RecognitionController::calculatePointDistances(PXCFaceData::LandmarkPoint *_points, float * _distances, float worldx, float worldy, float worldz) {
	...
	for (int i = 0; i<78; i++)
	{
		...
		*(_distances + i) = root;
		...
		//wprintf_s(L"found landmark  %d\n", int(root));
	}
	//wprintf_s(L"found landmark  avg %10.8lf\n", root);
}

void RecognitionController::SetDetection(pxcI32 _faceId, PXCRectI32 _faceRectangle)
{
